...
1. Open `clients/httpShopPaymentsClient.ts`. This file, plus the environment redirect allowlist and composition, are the only things you touch in this task.
...
3. Implement **initiation** (starting a payment) exactly like this (this is the full, complete implementation — copy it, adjusting only the schema/path names already defined in this file):
   ...
   return initiation;
   ```
   `shopFetch` is the existing authenticated fetch helper already used by other real clients in this codebase — reuse it, do not write a new fetch wrapper. `Idempotency-Key` (a header value that lets the server recognize a retried request as "the same request", so repeating it has no extra effect — this is what "idempotent" means) must be sent on every initiation call, so a retried initiation never starts a second, duplicate payment.
4. `assertAllowedPaymentRedirect(initiation.redirectUrl)` is an existing helper (or one you add alongside the environment redirect allowlist if it does not exist yet) that checks the redirect URL's scheme and host against an allowlist before the app navigates to it. Call it on every redirect URL you receive, before using that URL anywhere.
...
10. Open the file that defines `createShopClients()` (the composition function that wires each capability to either its mock or real HTTP implementation). Change **only** the `payments` slot so it points at your new `httpShopPaymentsClient.ts` implementation. Leave every other slot pointing at its mock; those get switched in their own later tasks.
