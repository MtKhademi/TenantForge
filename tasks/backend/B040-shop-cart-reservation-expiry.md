...
3. Any successful add/update/delete extends the lease. A read does not. Checkout summary and order creation first call `EnsureActiveAsync` (this is the only name for that method — an earlier draft of this Spec called it `ExpireIfNeededAsync`; that name is wrong, do not create it); an expired cart returns RFC7807 status `410`, `type=shop_cart_expired`.
...
`Tsid` here is the TSID type (a sortable numeric string ID — see `TenantForge.BuildingBlocks`) already used for IDs elsewhere in the module. `CartLeaseResult` is a small result type you define in the same file, with exactly this shape:

```csharp
internal enum CartLeaseOutcome { StillActive, JustExpired, AlreadyExpired, Converted }

internal sealed record CartLeaseResult(CartLeaseOutcome Outcome, ShopCart? Cart);
```

`Cart` is the current row after the call, or `null` when no cart matched `(tenantId, cartId)`. Do not rename these members and do not add a fifth outcome.
...
5. Re-read `docs/modules/SHOP.md` and check every routes/entities/config/auth/tests
   ...
   `docs/learning/B040-<slug>.md` learning note.
...
5. **Risks, blockers and follow-up.** Anything you could not verify, any
   ...
   (F058) must know. Write "None." if there is genuinely nothing.
