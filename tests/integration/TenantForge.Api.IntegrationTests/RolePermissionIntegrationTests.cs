...
public class RolePermissionIntegrationTests(RolePermissionDbFixture db) : IDisposable
{
    ...
    [Fact]
    public async Task Catalog_ReturnsEveryModulesKnownPermissionKeys()
    {
        ...
        // B035/B039/B042: the catalog aggregates every registered module
        // contributor — IAM's four keys plus Shop's five (B039 adds
        // Shop.Settings.Manage, B042 adds Shop.Orders.View/Manage) — into one
        // cross-module union.
        Assert.Equal([
            "IAM.Audit.View",
    ...
    public async Task Owner_CanCreateReloadAssignAndResolveCustomRole()
    {
        ...
        var create = await client.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", new
        {
        ...
        var assign = await client.PutAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/members/{TsidId.Format(tenant.MemberMembershipId)}/roles/{roleId}", null);
        Assert.Equal(HttpStatusCode.OK, assign.StatusCode);

        using var memberClient = CreateClient();
        Authorize(memberClient, tenant.MemberAccountId);
        var resolved = await memberClient.GetAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/me/permissions");
        ...
        var reload = await client.GetAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles");
        ...
        Assert.Contains(TsidId.Format(tenant.MemberMembershipId), reloadedRole.GetProperty("memberIds").EnumerateArray().Select(item => item.GetString()));
    }
    ...
    public async Task SameRoleName_IsTenantScoped()
    {
        ...
        Assert.Equal(HttpStatusCode.Created, (await firstClient.PostAsJsonAsync($"/api/tenants/{TsidId.Format(first.TenantId)}/roles", new
        {
            name = "Invitation Viewer",
            permissionKeys = new[] { "IAM.Invitations.View" }
        })).StatusCode);
        Assert.Equal(HttpStatusCode.Created, (await secondClient.PostAsJsonAsync($"/api/tenants/{TsidId.Format(second.TenantId)}/roles", new
        {
            name = "Invitation Viewer",
    ...
    public async Task InvalidRoleRequests_Return400_AndDuplicateNameReturns409()
    {
        ...
        var invalid = await client.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", new
        {
            name = "",
        ...
        var obsolete = await client.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", new
        {
            name = "Old Permission",
        ...
        Assert.Equal(HttpStatusCode.Created, (await client.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", request)).StatusCode);
        Assert.Equal(HttpStatusCode.Conflict, (await client.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", request)).StatusCode);
    }
    ...
    public async Task NonMemberAndNonManager_AreDeniedForRoleManagement()
    {
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await outsiderClient.GetAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles")).StatusCode);

        using var memberClient = CreateClient();
        Authorize(memberClient, tenant.MemberAccountId);
        Assert.Equal(HttpStatusCode.Forbidden, (await memberClient.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", new
        {
            name = "Denied",
            permissionKeys = new[] { "IAM.Invitations.View" }
        })).StatusCode);
    ...
    public async Task RolesManageMember_CanManageRoles_WithoutPlatformAccess()
    {
        ...
        var createManager = await ownerClient.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", new
        {
            name = "Role Manager",
        ...
        Assert.Equal(HttpStatusCode.OK, (await ownerClient.PutAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/members/{TsidId.Format(tenant.MemberMembershipId)}/roles/{managerRoleId}", null)).StatusCode);

        using var memberClient = CreateClient();
        Authorize(memberClient, tenant.MemberAccountId);
        var createManaged = await memberClient.PostAsJsonAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles", new
        {
            name = "Audit Reader",
            permissionKeys = new[] { "IAM.Audit.View" }
        });
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await memberClient.GetAsync($"/api/platform/users?tenantId={TsidId.Format(tenant.TenantId)}")).StatusCode);
    ...
    public async Task OwnerAndOrdinaryMemberResolvedPermissionsMatchServerDecisions()
    {
        ...
        // B035/B039/B042: an Owner's resolved union spans every registered
        // module's known keys — IAM's four plus Shop's five (B039 adds
        // Shop.Settings.Manage, B042 adds Shop.Orders.View/Manage) — not just
        // IAM's own. A plain Member still resolves nothing (no role grant)
        // and is denied the permission-gated reads.
        Assert.Equal([
            "IAM.Audit.View",
            "IAM.Invitations.Create",
            "IAM.Invitations.View",
        ...
        var memberPermissions = await memberClient.GetFromJsonAsync<JsonElement>($"/api/tenants/{TsidId.Format(tenant.TenantId)}/me/permissions");
    ...
    public async Task TenantAuthorizationRejectsRemovedMembershipSuspendedTenantAndDisabledAccount()
    {
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await memberClient.GetAsync($"/api/tenants/{TsidId.Format(tenant.TenantId)}/roles")).StatusCode);
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await suspendedClient.GetAsync($"/api/tenants/{TsidId.Format(suspended.TenantId)}/roles")).StatusCode);
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await disabledClient.GetAsync($"/api/tenants/{TsidId.Format(disabled.TenantId)}/roles")).StatusCode);
    }
    ...
    public async Task LastAdministratorProtection_EvaluatesDistinctFinalActiveAccounts()
    {
        ...
        var tenantId = TsidId.Format(tenantEntity.Id);
        var adminMembershipId = TsidId.Format(adminMembership.Id);
        var adminRoleId = TsidId.Format(adminRole.Id);
        var duplicateAdminRoleId = TsidId.Format(duplicateAdminRole.Id);
    ...
    public async Task ConcurrentLastAdministratorDemotions_DoNotBothSucceed()
    {
        ...
        var tenantId = TsidId.Format(tenantEntity.Id);
        var membershipId = TsidId.Format(membership.Id);
        var firstRoleId = TsidId.Format(firstRole.Id);
        var secondRoleId = TsidId.Format(secondRole.Id);
