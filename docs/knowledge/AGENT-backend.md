...
| API docs | Scalar UI at `/scalar/v1`, OpenAPI document at `/openapi/v1.json` (host-only; `AddOpenApi` + `MapOpenApi` + `MapScalarApiReference` in `Program.cs`; a document transformer tags every operation IAM / Shop / Host by path so the reference groups per module) |
