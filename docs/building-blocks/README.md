...
| Pagination (`PaginationSupport`, `PaginationQuery`, `PaginationMetadata`) | `src/modules/iam/TenantForge.Modules.Iam/features/pagination/` (`internal`) | Couples HTTP query binding, validation text, `IQueryable` and EF execution — not a boundary-neutral contract |
